...
void Handler<T>::makeInvertedIndex(Block block)
{
    ...
    unsigned int entriesOffset = _hostInput.starts[block.startID];
    unsigned int entriesSize = _hostInput.starts[block.endID] + _hostInput.sizes[block.endID] - entriesOffset;
...
//                intersect(*indexedBlock, *probeBlock);
...
{
    DeviceTiming::EventPair* allocateOutput = _deviceTimings.add("Allocate intersection & output arrays", 0);
    _deviceIntersection.init(_blockSize * _blockSize);
...
{
    DeviceTiming::EventPair* clearFilterSpace = _deviceTimings.add("Clear intersection space", 0);
    _deviceIntersection.zero();
...
{
    DeviceTiming::EventPair* freeOutput = _deviceTimings.add("Free intersection arrays & output pairs space", 0);
    _deviceIntersection.free();
    _devicePairs.free();
...
template <class T>
void Handler<T>::intersect(Block indexedBlock, Block probeBlock)
{
    DeviceTiming::EventPair* intersection = _deviceTimings.add("Calculating intersection between sets", 0);
    calculateIntersection<<<_grid, _threads>>>(
            _deviceIndex,
            indexedBlock,
            probeBlock,
            _deviceInput,
            _binaryJoin ? _deviceForeignInput : _deviceInput,
            _deviceIntersection,
            _blockSize,
            _threshold,
            _binaryJoin);

    _deviceTimings.finish(intersection);
}
...
void Handler<T>::verify(Block indexedBlock, Block probeBlock)
{
    ...
    verifyPairs<<<_grid, _threads>>>(indexedBlock,
            probeBlock,
            _deviceInput,
            _binaryJoin ? _deviceForeignInput : _deviceInput,
            _devicePairs,
