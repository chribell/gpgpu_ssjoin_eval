...
int gpu(int argc, char **argv) {
	...
	if (argc != 7) {
		cerr << "Wrong parameters. Correct usage: <executable> <input_file> <threshold> <output_file> <number_of_gpus> <size of blocks> <count>" << endl;
		exit(1);
	}
		...
		for (int i = gpuid; i < block_num; i+= gpuNum) {
			...
			for (int j = 0; j <= i; j++) { // calculate similarity between indexed sets and probe sets
				...
				int totalSimilars = findSimilars(index, threshold, &dev_vars, similar_pairs, probe_block_start,
						probe_block_size, probes_offset, entries_block_start, entries_block_size, block_size, aggregate, deviceTiming);
